...
func _ready():
	...
	#Resize Window
	OS.set_window_size(Vector2(500.0, 500.0));
...
func _inverseKinematic(delta):
	...
	J.x      = Vector2(-l1*sin(d1) - l2*sin(d1 + d2) - l3*sin(d1 + d2 + d3), l1*cos(d1) + l2*cos(d1 + d2) + l3*cos(d1 + d2 + d3))
	J.y      = Vector2(-l2*sin(d1 + d2) - l3*sin(d1 + d2 + d3),              l2*cos(d1 + d2) + l3*cos(d1 + d2 + d3))
	J.origin = Vector2(-l3*sin(d1 + d2 + d3),                                l3*cos(d1 + d2 + d3))

	I = J.translated(Vector2(1, 1)) * (J * J.translated(Vector2(1, 1))).affine_inverse()
	...
	#TODO : insert code here
	#var angle_update = Transform2D()
	var angle_update = I * (targetPos - currentPos)
	print(angle_update)
	#update angles:
	#TODO : insert code here
